...
ValueNotifier<int> currentPageForCashCollectionReport = ValueNotifier(1);

class CashCollectionReport extends StatefulWidget {
  const CashCollectionReport({
    super.key,
  });

  @override
  State<CashCollectionReport> createState() => _CashCollectionReportState();
}

class _CashCollectionReportState extends State<CashCollectionReport> with WidgetsBindingObserver {
  CashCollectionBloc? bloc;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance?.addObserver(this);
    context.read<CashCollectionBloc>().mainSearchStream.listen((value) {
      if (value.isEmpty) {
        _controller.clear();
  ...
  void didChangeDependencies() {
    ...
    bloc ??= Provider.of<CashCollectionBloc>(context);
    bloc?.clearSreams();
...
class _Table extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final searchBloc = Provider.of<CashCollectionBloc>(context);
    ...
    return StreamBuilder(
        ...
        builder: (context, settingsSnapshot) {
          ...
          return StreamBuilder(
              stream: searchBloc.getTicketRespStream,
              builder: (context, snapshot) {
                print('1231232131231312312321 ${snapshot.data}');
                //print(snapshot.data);
                                                  ...
                                                  ...List.generate(allTickets?.data?.totalPages ?? 0, (index) {
                                                    return Padding(
                                                      ...
                                                      child: ValueListenableBuilder(
                                                        valueListenable: currentPageForCashCollectionReport,
                                                        builder: (context, ix, _) {
                                                          return Container(
                                                            height: 22,
                                                            width: 22,
                                                            decoration: BoxDecoration(
                                                              color: index + 1 != currentPageForCashCollectionReport.value ? Colors.grey : Colors.purple[400],
                                                            ...
                                                            child: InkWell(
                                                              onTap: () async {
                                                                ...
                                                                currentPageForCashCollectionReport.value = index + 1;
...
class _CustomExpansionTileState extends State<_CustomExpansionTile> {
  ...
  Widget build(BuildContext context) {
    ...
    final bloc = Provider.of<CashCollectionBloc>(context);
    return Padding(
  ...
  Future<void> _selectDate(BuildContext context, ValueNotifier<DateTime?> selectedDate) async {
    final bloc = Provider.of<CashCollectionBloc>(context, listen: false);
    filterValue.add('');
